...
import { fetchDesignPageL2Data, type FAQsListItem } from "@/store/Slice/UxDesgin/DesginPageThunk";
...
const FAQSection: React.FC = () => {
  ...
  // Conditionally render based on faqs flag
  const shouldShowSection = data?.faqs === 1;

  // Use API data if available, otherwise use defaults
  const heading = data?.faqs_heading || '<div class="ql-editor read-mode"><p>Frequently Asked <strong>Questions</strong></p></div>';

  // Convert faqs_list to FaqItem format
  const faqs: FaqItem[] = useMemo(() => {
    if (data?.faqs_list && data.faqs_list.length > 0) {
      return data.faqs_list
        .filter((item: FAQsListItem) => item.title && item.description)
        .map((item: FAQsListItem) => ({
          q: item.title || '',
          a: item.description || '',
        }));
    }
     ...
     py-10 md:py-[60px] lg:py-[120px]">
      <div className="grid md:gap-12 sm:gap-[32px] gap-[24px] lg:grid-cols-10 lg:gap-16">
        ...
        <div className="lg:col-span-4 flex flex-col lg:items-start items-center justify-center text-left 2xl:min-w-[480px]">
          {heading ? (
            <>
              <div className="2xl:text-[96px] xl:text-[82px] md:text-[68px] sm:text-[48px] text-[32px] text-left lg:block hidden">
                <ParsedHtml htmlContent={heading} as="h2" />
              </div>
              <div className="2xl:text-[96px] xl:text-[82px] md:text-[68px] sm:text-[48px] text-[32px] text-center lg:hidden block">
                <ParsedHtml htmlContent={heading} as="h2" />
              </div>
            </>
          ) : (
