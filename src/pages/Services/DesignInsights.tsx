...
export default function DesignInsights({ heading, description, buttonData }: DesignInsightsProps) {
  ...
  const posts: Post[] = useMemo(() => {
    ...
    // Get all allowed tags from insight_tags
    const allowedTags: string[] = [];
    if (data?.insight_tags && Array.isArray(data.insight_tags)) {
      allowedTags.push(
        ...data.insight_tags
          .map((item) => item.tag)
  ...
  }, [data?.insights_list, data?.insight_tags]);

  // Conditionally render based on service_insights flag
  const shouldShowSection = data?.service_insights === 1;

  // Use API data for heading, description, and button
  const headingText = heading || data?.service_insights_heading;
  const descriptionText = description || data?.service_insights_description;
  const buttonText = buttonData || data?.service_insights_buttondata || "View All Insights";

  // Don't render if service_insights is 0
