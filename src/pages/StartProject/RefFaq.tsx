...
const RefFaq: React.FC = () => {
  const [openIndex, setOpenIndex] = useState<number | null>(null); // all closed

  // Dummy data
  const faqs: FaqItem[] = [
    { q: "Can I submit an early-stage idea?", a: "Absolutely. We often collaborate with founders and teams from the idea stage." },
    { q: "What if I don’t have a full RFP yet?", a: "We typically start with a pilot project, which can range from a few hours to a few weeks, depending on the scope and complexity." },
    ...
    { q: "Can you sign an NDA before I submit?", a: "Founded in 2012, Hostripples is an Indian web hosting company headquartered in Nashik, Maharashtra. We provide best web hosting services globally with data centers in six countries including, India, the USA, the UK, Canada, Australia and Singapore. We are proud to be one of the top-rated hosts in the industry and are dedicated to raising the bar every day. You can learn more about us." },
  ];
     ...
     py-10 md:py-[60px] lg:py-[120px]">
      <div className="grid md:gap-12 sm:gap-[32px] gap-[24px] lg:grid-cols-10 lg:gap-16 ">
        ...
        <div className="lg:col-span-4 flex flex-col lg:items-start items-center justify-center text-left 2xl:min-w-[480px]">
          <h2 className="2xl:text-[72px] xl:text-[70px] md:text-[68px] sm:text-[48px] text-[32px] text-left lg:block hidden ">
            REP
            <span className=" lg:font-semibold"> FAQs</span>
          </h2>
          <h2 className="2xl:text-[72px] xl:text-[70px] md:text-[68px] sm:text-[48px] text-[32px] text-center lg:hidden block">
            REP FAQs
          </h2>
        </div>

        {/* Right — 60% */}
        <div className="lg:col-span-6 lg:pl-[50px] xl:pl-[0px]">
...
const AccordionItem: React.FC<AccordionItemProps> = ({ q, a, isOpen, onToggle }) => {
  ...
  return (
    <div className={`transition-all duration-300 ease-in-out ${isOpen ? "border border-[#E5E7EA]/50 p-[12px] sm:p-[16px] md:p-[20px] lg:p-[24px] xl:p-[4px]" : "border-none px-[12px] py-[8px] sm:px-[16px] sm:py-[10px] md:px-[20px] md:py-[12px] lg:px-[24px] lg:py-[16px] xl:px-[4px] xl:py-[2px]"}`}>
      <button
        id={`faq-btn-${uid}`}
        className="flex w-full items-center justify-between gap-5 py-4 text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-neutral-800 transition-colors duration-200 hover:bg-gray-50/50 rounded-lg"
