...
const Reports = () => {
  ...
  const heading = stripHtml(data?.report_heading, "Reports");
  const subheading = stripHtml(
    data?.report_subheading,
    "Download sample pilot documents. Share with teams. Decide with clarity."
  );
  const title = stripHtml(data?.report_title, "UX in the Age of Automation 2025 Report");
  ...
  return (
    <main className="px-4 sm:px-6 md:px-12 lg:px-20 xl:px-28 2xl:px-[200px] py-10 md:py-14 lg:py-16 xl:py-20 2xl:py-24">
      <section className="w-full">
        <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl text-center font-semibold mb-4 text-black">
          {heading}
        </h1>
        <p className="text-[var(--hero-text)] text-[14px] md:text-2xl font-urbanist text-center mb-10">
        {subheading}
        </p>
